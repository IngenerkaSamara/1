...
//    HDC coffee = txLoadImage("coffee.bmp");
    ...
    Button selects[6];
    selects[0] = {50, 270, 250,  300, "Выбрать", nullptr, vzlom, vzlom2};
    selects[1] = {300, 270, 500, 300, "Выбрать", nullptr, zheka, zheka2};
    selects[2] = {550, 270, 750, 300, "Выбрать", nullptr, magnum};
    selects[3] = {50, 500, 250,  550, "Выбрать", nullptr, floppa};
    selects[4] = {300, 500, 500, 550, "Выбрать", nullptr, txLoadImage("garden.bmp"), txLoadImage("garden2.bmp")};
    selects[5] = {550, 500, 750, 550, "Выбрать", nullptr, mama};
    ...
    while(!GetAsyncKeyState(VK_ESCAPE)){
        ...
        //Инвентарь
            ...
            drawQuest("В 1х Покупка на целый район и банду.\n\n И ваще ЧЕ ХАЧУ ТО И ДеЛАЮ");
        }
        else if(PAGE == "Quest2"){
            drawQuest("ОНИ ПРИКОЛЬНЫЕ ПАЦАНЫ \n И ЧЕ ХАЧУ ТО И ДЕЛАЮ!!!!\n И НИКТО МНЕ НЕ ПОМЕШАЕТ!!!!");
        }
        else if(PAGE == "Quest3"){
            drawQuest("КОНЕЧНО ЖЕ НЕТ И КОГДА ВЫ ЭТО ЧИТАЕТЕ \n К ВАМ УЖЕ СОБИРАЕТСЯ НАРЯД");
        }
        else if(PAGE == "Quest4"){
            drawQuest("КОНЕЧНО ЖЕ ДА и  Вся ИгРа написона Под Кайфом ТЯЖЕЛЫХ НАРКОТИКОВ\n и У АВТОРА ДОМА \n 18 КГ ГЕРОИНА и ПОДДЕЛЬНЫЕ ДЕНЬГИ\n ВОТ АДРЕСС 19414, Сумська область, місто Суми, пров. Володимирська, 89 !!!");
        }
